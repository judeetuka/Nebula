...
//! Shared helpers for the Payment Processor plugin.
//!
...
/// Read a value from the plugin's persisted key-value store with a larger
/// buffer. Used for transaction logs and payment history.
pub fn get_state_large(key: &str) -> Result<Option<String>, String> {
    let ctx = with_ctx()?;
    let mut val_buf = vec![0u8; 262144]; // 256 KiB
    let ret = (ctx.get_state)(
        ctx.host_data,
        key.as_ptr(),
        key.len(),
