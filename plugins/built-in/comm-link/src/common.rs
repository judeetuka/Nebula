//! Shared helpers for the Comm-Link plugin.
//!
...
pub fn invoke(capability: &str, args: &str) -> Result<String, String> {
    ...
    // The `method` parameter in `platform_invoke` is unused for android:
...
///
/// Returns `None` if the key was not found, `Err` on internal error.
...
///
/// Uses `std::time::SystemTime` which is available on all platforms including
/// Android via the NDK.
pub fn now_ms() -> i64 {
...
/// Generate a simple message ID based on timestamp and a counter.
///
/// Not cryptographically unique, but sufficient for queue deduplication within
/// a single plugin instance. The format is `cl-<epoch_ms>-<counter>`.
pub fn generate_message_id() -> String {
    use std::sync::atomic::AtomicU64;
    static COUNTER: AtomicU64 = AtomicU64::new(0);
    let seq = COUNTER.fetch_add(1, Ordering::Relaxed);
    format!("cl-{}-{seq}", now_ms())
}
