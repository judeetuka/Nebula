...
pub fn feature_not_compile(feature: &str) -> ! {
    panic!(
        "The feature '{}' is not compiled in this binary. Please re-compile nebula-server",
        feature
    )
}
...
pub fn feature_neither_compile(feature1: &str, feature2: &str) -> ! {
    panic!(
        "Neither of the feature '{}' or '{}' is compiled in this binary. Please re-compile nebula-server",
        feature1, feature2
    )
}
...
/// Connect to a TCP address directly (no proxy support in NEBULA server).
pub async fn tcp_connect_direct(addr: &AddrMaybeCached) -> Result<TcpStream> {
