...
class ClusterApiSource implements ClusterRemoteSource {
  ...
  ClusterApiSource({required this.baseUrl, http.Client? client})
  ...
  Future<List<Cluster>> getClusters() async {
    ...
    if (response.statusCode != 200) {
      throw ClusterFailure(
        'Failed to fetch clusters: ${response.statusCode}',
      );
    }
  ...
  Future<Cluster> createCluster({required String name}) async {
    ...
    if (response.statusCode != 200 && response.statusCode != 201) {
      throw ClusterFailure(
        'Failed to create cluster: ${response.statusCode}',
      );
    }
  ...
  Future<List<NodeInfo>> getClusterNodes({required String clusterId}) async {
    ...
    if (response.statusCode != 200) {
      throw ClusterFailure(
        'Failed to fetch nodes: ${response.statusCode}',
      );
    }
    final List<dynamic> data = jsonDecode(response.body) as List<dynamic>;
    return data
